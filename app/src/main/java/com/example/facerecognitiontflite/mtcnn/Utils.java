...
public class Utils {
    // Copy the bitmap image and set isMutable=true
    ...
    // Flip alone diagonal
    // Flip diagonally. The size of data was originally h*w*stride, but it becomes w*h*stride after flipping
    ...
    public static Rect[] boxes2rects(Vector<Box> boxes){
        ...
        for (int i=0; i<boxes.size(); i++)
            if (!boxes.get(i).deleted) r[idx++] = boxes.get(i).transform2Rect();
        return r;
    }
    ...
    static public Bitmap resize(Bitmap _bitmap, int new_width){
        ...
        Bitmap bitmap = Bitmap.createBitmap(_bitmap,0,0,
                _bitmap.getWidth(), _bitmap.getHeight(), matrix,true);
        return bitmap;
    }
}